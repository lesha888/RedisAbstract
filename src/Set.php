...
use RedisAbstract\Exception\NotSupportedException;
use RedisAbstract\Exception\SerializerIsNotSupportedException;
 ...
 *
 * <pre>
 * $set = new RedisAbstract\Set("mySet");
 * $set->add(1);
 * $set->add(2);
 * $set->add(3);
 *
 * $otherSet = new RedisAbstract\Set("myOtherSet");
 * $otherSet->add(2);
 *
...
class Set extends IterableEntity
{
     ...
     * @param mixed $set , $set2 The sets to compare to, either RedisAbstract\Set instances or their names
    ...
    public function copyFrom($data)
    {
        if (\is_array($data) || ($data instanceof Traversable)) {
            ...
            if ($data !== null) {
                throw new \RedisAbstract\Exception\RedisException('List data must be an array or an object implementing Traversable.');
            }
        }
    }
